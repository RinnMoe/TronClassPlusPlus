# TronClass++

TronClass++ 是一个畅课功能增强插件  
提供仅预览文件下载、批量下载和视频播放辅助等功能。


## 安装

请见[使用教程](https://www.doubao.com/thread/w673a6ddbec450a73)

## 说明

本扩展理论上兼容所有畅课(TronClass)平台，可在 Chromium 系及 Safari 浏览器使用。

> 免责声明：本工具仅供学习交流之用途，请尊重版权。不支持、不鼓励任何利用本工具进行不正当盈利的行为，不当使用本工具造成的后果自负。

## 参考项目
[XMUFD](https://github.com/KrsMt-0113/XMUFD) 、
[tronclass-hack](https://github.com/CUC-Life-Hack/tronclass-hack)
